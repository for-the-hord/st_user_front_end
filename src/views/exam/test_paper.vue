    ...
    <d2-container>
        <div class="Aone-page" style="overflow: auto">
            <p class="" style="color:gray;cursor: pointer;">等级考试 > <span @click="goback2()"> {{ lastTitle }} </span> >
                <span style="color:black">在线评测</span>
            </p>
            <br />
            <br />
            <div class="heads">
                ...
                <p style="color:gray">
                    试卷说明：
                    本次测评单选题{{ single_num }}个，共{{ single_num }}分，多选题{{ multi_num }}个，共{{ multi_score }}分，判断题{{ judge_num
                    }}个，共{{ judge_score }}分。
                </p>
            ...
            <div class="body-lefts">
                <div v-for="(item, index) in all_question">
                    ...
                    <div v-for="(item2, index2) in item.question.options">
                        <div v-if="item.question.type == 1" class="option-wrap">
                            <el-radio class="new_radio2" v-model="item.point" :label="item2.option">{{ item2.content }}
                            </el-radio>
                        </div>
                        <div v-if="item.question.type == 2">
                            <el-checkbox class="new_radio2" v-model="item.point" :label="item2.option">{{ item2.content
                            }}
                            </el-checkbox>
                        </div>
                        <div v-if="item.question.type == 3">
                            <el-radio class="new_radio2" v-model="item.point" :label="item2">{{ item2.content }}
                            </el-radio>
                        </div>
                    </div>
                ...
                <p class="pointer"  style="text-align:center;color:#0a87b6">
                  <span v-if="jiazai" @click="add_list()" >      点击加载更多</span> 
                  <span v-if="!jiazai">  <i class="el-icon-check"> </i>  加载完毕</span> 
                </p>
            ...
            <div class="body-rights">
                ...
                <br />
                <br />
                <div class="dati" style="max-height:400px">
                    <p class="flex_center_starts2">
                    ...
                    <div>
                        <span v-for="(item, index) in all_question" style="margin-left:20px"> <span
                                :class="setClass(item.point) ? 'done' : 'doing'" style="width:30px">{{ index + 1
                                }}</span></span>
...
<script>
import { target, getid, target2, paperQuestion, postpaper } from "./big.js";
export default {
  name: "mine",
  ...
  created() {
    ...
    if (this.$route.query.state == 1) {
      ...
      getid({ paper_id: this.paperId }).then((res) => {
        this.exam_id = res.data;
      });
    } else {
  ...
  methods: {
    ...
    getNewArray(arr, size) {
      ...
      this.all_question2 = result;
      console.log(this.all_question2, "all_question2");
    },
    add_list() {
        
      this.all_count += 1;
      if(this.all_count==this.all_question2.length){
        this.jiazai = false
      }
    //   this.all_question.push(this.all_question2[this.all_count]);
     this.all_question= [...this.all_question,...this.all_question2[this.all_count]]
      console.log('添加数据',this.all_count)
      console.log('添加数据all_question2',this.all_question2[this.all_count])
      console.log('添加数据all_question',this.all_question )
    },
    setClass(x) {
      if (x != "") {
        return true;
        ...
        if (res.code == 200) {
          ...
          abc.map((e) => {
            ...
            if (e.question.type == 3) {
              e.point = [];
              e.question.options = ["是", "否"];
            }
          });
          this.all_question_copy = abc;
          this.all_question = abc.slice(0, 10);
          console.log(this.all_question, "all_question");
          console.log(this.all_question_copy, "all_question_copy");
          this.getNewArray(this.all_question_copy, 10);
    ...
    go_points() {
      ...
      if (!this.bianli(records)) {
        ...
        postpaper(this.exam_id, { records: records }).then((res) => {
          if (res.code == 200) {
