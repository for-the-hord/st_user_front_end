<template>
  <d2-container>
    <div class="Aone-page">
      ...
      <div class="jigou">
        <el-form :inline="true" :model="formInline" :rules="rules"  ref="formInline" class="demo-form-inline">
          ...
          <el-form-item label="机构状态：">
            <el-select v-model="formInline.state" prop="state" clearable placeholder="请选择机构状态">
          ...
          <el-form-item label="机构类型：">
            <el-select v-model="formInline.type"  prop="type" clearable placeholder="请选择机构类型">
              <!-- 0-未知，1-企业，2-高校，3-政府，4-科研单位 -->
              <el-option label="全部" value="-1"></el-option>
              <el-option label="企业" value="1"></el-option>
              <el-option label="高校" value="2"></el-option>
              <el-option label="政府" value="3"></el-option>
              <el-option label="科研单位" value="4"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="机构证书：">
            <el-select v-model="formInline.is_cert" prop="is_cert" clearable placeholder="请选择证书状态">
              <el-option label="全部" value="-1"></el-option>
      ...
      <div class="qd-table" style="position: relative;margin:0 auto">
        ...
        >
          ...
          <el-table-column prop="name" label="机构名称" :cell-style="timeStyle">
            <template slot-scope="scope">
              <span style="cursor: pointer"> {{ scope.row.name_cn || scope.row.name }}</span>
            </template>
          </el-table-column>
          <el-table-column prop="name" label="机构类型" :cell-style="timeStyle">
            <template slot-scope="scope">
              <span style="cursor: pointer" v-if="scope.row.type == 1"> 企业</span>
              <span style="cursor: pointer" v-if="scope.row.type == 2"> 高校</span>
              <span style="cursor: pointer" v-if="scope.row.type == 3"> 政府</span>
              <span style="cursor: pointer" v-if="scope.row.type == 4"> 科研单位</span>
            </template>
          </el-table-column>
          <el-table-column prop="name" label="机构人数" :cell-style="timeStyle">
            <template slot-scope="scope">
              <span style="cursor: pointer"> {{ scope.row.total_user }}</span>
            </template>
          </el-table-column>
          <el-table-column prop="name" label="职称总数" :cell-style="timeStyle">
            <template slot-scope="scope">
              <span style="cursor: pointer"> {{ scope.row.title_user }}</span>
            </template>
          </el-table-column>

          <el-table-column prop="name" label="统一社会信用代码" :cell-style="timeStyle">
            <template slot-scope="scope">
              <span style="cursor: pointer"> {{ scope.row.code }}</span>
            </template>
          </el-table-column>
          <el-table-column prop="name" label="审核状态" :cell-style="timeStyle">
            <template slot-scope="scope">
              <!-- 审核状态：0-未审核，1-未通过，2-已审核 -->
              <span style="cursor: pointer" v-if="scope.row.state == 0"> 未审核 </span>
              <span style="cursor: pointer" v-if="scope.row.state == 1"> 未通过 </span>
              <span style="cursor: pointer" v-if="scope.row.state == 2"> 已审核 </span>
            </template>
          </el-table-column>
          <el-table-column prop="certificate" label="机构证书" :cell-style="timeStyle">
            <template slot-scope="scope">
              <!-- 审核状态：0-未审核，1-未通过，2-已审核 -->
              <span style="cursor: pointer" v-if="scope.row.certificate == 0"> 未生成 </span>
              <span style="cursor: pointer" v-if="scope.row.certificate == 1"> 已生成 </span>
             </template>
          </el-table-column>

          <el-table-column label="操作">
            <template slot-scope="scope">
              <el-button type="warning" size="mini" round @click="spider_put(scope.row)" disabled v-if="scope.row.state==2">审核</el-button>
              <el-button type="warning" size="mini" round @click="spider_put(scope.row)" v-else>审核</el-button>
              <el-button type="primary" size="mini" round @click="spider_put2(scope.row)">查看</el-button>
            </template>
          </el-table-column>
        </el-table>
      </div>
    ...
    <qd-dialog
      ...
      :nofooter="true"
    >
    <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="60px" style="padding:20px;width: 80%;margin: 0 auto;" class="demo-ruleForm">
            <el-form-item label="机构名称：" prop="name">
              <el-input v-model="ruleForm.name" disabled></el-input>
            </el-form-item>
            <el-form-item label="机构描述：" prop="description">
              <el-input type="textarea" :rows="3" v-model="ruleForm.description" disabled></el-input>
            </el-form-item>
            ...
            <el-form-item label="所在区域：" prop="pro">
              <el-select clearable @change="get_city" v-model="ruleForm.pro" placeholder="请选择一级区域"
                style="width:30%;float:left;margin-right:10px" disabled>
                <el-option v-for="item in pro_list" :label="item.name" :code="item.code" :value="item.code">
                </el-option>
              </el-select>
              <el-select clearable v-if="if_city" @change="get_xian" v-model="ruleForm.city" placeholder="请选择二级区域"
                style="width:30%;float:left;margin-right:10px" disabled>
            ...
            <el-form-item label="职称信息：">
              ...
              <div style="width: 100%; height: 40px; margin-top: 10px" v-for="(item, index) in work_list">
                <div style="width: 35%; float: left">
                  <el-select disabled v-model="item.title_type" placeholder="请选择职称类型">
                ...
                <div style="width: 25%; float: left;margin-left:10px">
                  <el-input type="number" disabled v-model="item.title_user"></el-input>
                </div>
      ...
      <el-form ref="form"  style="padding:20px;width: 80%;margin: 0 auto;" :model="form" label-width="80px">
        <el-form-item label="审核意见">
          <el-input type="textarea" :rows="3" v-model="form.state_remark"></el-input>
        </el-form-item>
        <el-form-item>
    ...
    <qd-dialog
      :title="dialogInfo3.title"
      :visible.sync="dialogInfo3.visible"
      :width="dialogInfo3.width"
      :bt-loading="dialogInfo3.btLoading"
      @handleOk="submitFormhandle()"
      @quxiao="quxiao()"
      :isOk="isok"
      :nofooter="true"
    ...
    <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="60px" style="padding:20px;width: 80%;margin: 0 auto;" class="demo-ruleForm">
            ...
            <el-form-item>
              <el-button type="primary" @click="dialogInfo3.visible=false">关闭</el-button>
            </el-form-item>
          </el-form>
...
<script>
import { process2, process3, detail, new_area, new_area2 } from './big.js';
export default {
  name: 'mine',
  data() {
    return {
